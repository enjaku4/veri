RSpec.describe Veri::Session do
  ...
  describe ".active" do
    subject { described_class.active(authenticatable) }
    ...
    context "when authenticatable is nil" do
      ...
      it "returns only active sessions" do
        expect(subject).to contain_exactly(session1, session2)
      end

      context "when inactive session lifetime is set" do
        before { Veri::Configuration.configure { _1.inactive_session_lifetime = 5.minutes } }

        it "returns only not expired sessions" do
          expect(subject).to contain_exactly(session2)
        end
      end
    end
    ...
    context "when authenticatable is present" do
      ...
      it "returns only active sessions for the given authenticatable" do
        expect(subject).to contain_exactly(session1, session2)
      end
      ...
      context "when inactive session lifetime is set" do
        ...
        it "returns only not expired sessions for the given authenticatable" do
          expect(subject).to contain_exactly(session2)
        end
      end
    end
